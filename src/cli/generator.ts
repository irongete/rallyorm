...
export async function generateModels(options: IGeneratorOptions): Promise<void> {
    ...
    const types = await client.queryAll('TypeDefinition', {
        fetch: 'ObjectID,Name,ElementName,Attributes,Abstract,Parent',
        pagesize: 200
    });

    const validTypes = types.filter((t: any) => t.ElementName && t.Abstract === false);
    ...
    for (let i = 0; i < validTypes.length; i++) {
        ...
        const classContent = generateClassContent(className, typeDef, attributesResponse, enumMap);
        fs.writeFileSync(filePath, classContent, 'utf-8');

        classNames.push(className);
        indexExports.push(`export { ${className} } from './${fileName.replace('.ts', '.js')}';`);
    }

    const indexContent = `// Automatically generated by RallyORM CLI\n` +
        indexExports.join('\n') + '\n\n' +
        classNames.map(cn => `import { ${cn} } from './${cn.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase()}.js';`).join('\n') + '\n\n' +
...
function generateClassContent(
    ...
    const entityType = typeDef.ElementName;
    ...
    return `import { RallyEntity } from 'rallyorm';
